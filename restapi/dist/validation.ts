...
const ApiMessageKeys: string[] = ['message']
...
const ApiUserKeys: string[] = ['id', 'name', 'dob', 'gender', 'phoneNumber', 'isExist']
...
function modelApiUserFromJsonContent(name: string, value: any, knownKeys: Record<string, boolean> = {}): Api.User {
	...
	const result: Api.User = {
		...
		'dob': allowUndefined(stringFromJson)(`${name}.dob`, value['dob']),
		'gender': allowUndefined(enumApiUserGenderEnumFromJson)(`${name}.gender`, value['gender']),
		'phoneNumber': allowUndefined(stringFromJson)(`${name}.phoneNumber`, value['phoneNumber']),
...
function modelApiUserToJsonContent(name: string, value: Api.User, knownKeys: Record<string, boolean> = {}): ToJson<Api.User> {
	...
	const result: ToJson<Api.User> = {
		...
		'dob': allowUndefined(stringToJson)(`${name}.dob`, value['dob']),
		'gender': allowUndefined(enumApiUserGenderEnumToJson)(`${name}.gender`, value['gender']),
		'phoneNumber': allowUndefined(stringToJson)(`${name}.phoneNumber`, value['phoneNumber']),
...
export function enumApiUserGenderEnumFromJson(name: string, value: any): Api.User.GenderEnum {
	if (typeof value !== 'string' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected string got ${typeof value}`
	}

	if (value === 'male') {
		return Api.User.GenderEnum.Male
	}
	if (value === 'female') {
		return Api.User.GenderEnum.Female
	}

	throw `Unexpected enum value for Api.User.GenderEnum: ${value}`
}

export function enumApiUserGenderEnumToJson(name: string, value: Api.User.GenderEnum): Api.User.GenderEnum {
	if (typeof value !== 'string' || value === undefined || value === null) {
		throw `Invalid type for ${name}: expected string got ${typeof value}`
	}
